<?php
	...
	if (isset($_GET["membershipplan"])) {
		$membershipplan =  $_GET['membershipplan'];
	} else {
		$membershipplan = "";
	}
	...
	if (isset($_GET["email"])) {
		$email = trim(htmlentities(strip_tags($_GET['email'])));
	...
	if (isset($_GET["password"])) {
		$password = trim(htmlentities(strip_tags($_GET['password'])));
...
*/
	...
	if ($row) {
		...
		$sqlmembers=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO jpc_members (
			i_membershipplan,
			i_firstname,
			i_lastname,
			i_middlename,
		...
		$sqlmembers2 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM jpc_members");
		while ($rows=mysqli_fetch_array($sqlmembers2)) {
			if ($currentcount != 0) {
				...
				if ($memberid1 == $id) {
					if ($currentlevel > 0) {
						$sqlgeneaology=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO jpc_geneaology (
							i_memberid,
							i_uplineid,
							i_placementid,
							i_level,
		...
		$sqlmembers3 = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM jpc_members");
		while ($rows=mysqli_fetch_array($sqlmembers3)) {
			if ($currentcount2 != 0) {
				...
				if ($memberid3 == $id2) {
					if ($currentlevel2 > 0) {
						$sqlgeneaology2=mysqli_query($GLOBALS["___mysqli_ston"], "INSERT INTO jpc_geneaology2 (
							i_memberid,
							i_uplineid,
							i_sponsorid,
							i_level,
		...
		$your_email_subject = "jpc New Member Email Verification";

		$message_from_the_sender_of_your_website = "<html><body>" ;
		$message_from_the_sender_of_your_website .= "Dear " . $membername . ",<br><br>Thanks for signing up for jpc! We're excited to have you as a new member.<br><br>Copy and Paste or Click this link to your browser to confirm your email ->" . $serverdirectory . "verification/v.php?e=" . $emailaddress . "&v=" . $memberverification . " to continue registration process.<br><br>Regards,<br><br>Support Team";
		$message_from_the_sender_of_your_website .=  $video_link ;
		$message_from_the_sender_of_your_website .=  $video_message ;
		$message_from_the_sender_of_your_website .= "</body></html>";
